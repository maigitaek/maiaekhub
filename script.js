...
let prSortColumn = null;         // 'request_date' | 'requested_by' | 'details' | 'vendor' | 'status' | null
let prSortDirection = 'asc';     // 'asc' | 'desc'
...
let checkCostFile = null;        // File ที่เลือกไว้สำหรับเครื่องมือ CheckCost
let checkCostErrorRows = [];     // ผลลัพธ์แถวที่ตรวจพบข้อผิดพลาดล่าสุด (ใช้ตอนกดส่งออก Excel)
...
function applyTheme(theme) {
  ...
  if (metaTheme) metaTheme.setAttribute('content', theme === 'light' ? '#f3f6f7' : '#08090c');
}
...
function categoryHue(category) {
  if (!category) return 235; // ไม่มีหมวด -> ใช้โทนม่วง-น้ำเงินเหมือนสี accent หลักของเว็บ
  let hash = 0;
...
// ==================================================
// การ์ดสรุปข้อมูลงานขอราคาสไตล์ pixel game (กดที่แถวในตารางเพื่อดูภาพรวมโดยไม่ต้องเปิดฟอร์มแก้ไข)
// ==================================================
function openPrInfoCard(id) {
  const r = allPriceRequests.find(x => x.id === id);
  if (!r) return;

  const hue = categoryHue(r.requested_by);
  $('prPixelCard').style.setProperty('--pixel-hue', String(hue));

  $('prPixelRequestedBy').textContent = r.requested_by || '—';
  $('prPixelDate').textContent = formatPrDate(r.request_date);
  $('prPixelCardTitle').textContent = r.details || '—';

  const statusBadge = $('prPixelStatusBadge');
  statusBadge.dataset.status = r.status;
  statusBadge.dataset.id = r.id;
  statusBadge.textContent = PR_STATUS_LABEL[r.status] || r.status;
  ...
  const isStale = r.status !== 'done' && days !== null && days > STALE_PR_DAYS;

  $('prPixelOverdueRibbon').hidden = !isOverdue;
  $('prPixelStaleRibbon').hidden = !isStale;

  $('prPixelStats').innerHTML = `
    ${r.source ? `<div class="pixel-stat-row"><span class="pixel-stat-key">แหล่งที่มา</span><span class="pixel-stat-val">${PR_SOURCE_ICON[r.source] || ''}${escapeHtml(PR_SOURCE_LABEL[r.source] || r.source)}</span></div>` : ''}
    <div class="pixel-stat-row">
      <span class="pixel-stat-key">Vendor</span>
      <span class="pixel-stat-val">${r.vendor ? escapeHtml(r.vendor) : '<span class="pixel-stat-empty">—</span>'}</span>
    </div>
    <div class="pixel-stat-row">
      <span class="pixel-stat-key">หมายเหตุ</span>
      <span class="pixel-stat-val">${r.notes ? escapeHtml(r.notes) : '<span class="pixel-stat-empty">—</span>'}</span>
    </div>
    <div class="pixel-stat-row${isOverdue ? ' is-alert' : ''}">
      <span class="pixel-stat-key">อัปเดตสถานะล่าสุด</span>
      <span class="pixel-stat-val">${r.status_changed_at ? formatHoursTh(hoursSince(r.status_changed_at)) + 'ที่แล้ว' : '<span class="pixel-stat-empty">—</span>'}</span>
    </div>
    <div class="pixel-stat-row">
      <span class="pixel-stat-key">แก้ไขล่าสุด</span>
      <span class="pixel-stat-val">${r.last_edited_at ? formatPrDate(r.last_edited_at) : '<span class="pixel-stat-empty">—</span>'}</span>
    </div>
  `;

  const editBtn = $('prPixelEditBtn');
  editBtn.onclick = () => { closePrInfoCard(); openPrModal(r.id); };

  const copyBtn = $('prPixelCopyBtn');
  copyBtn.onclick = () => {
    const summary = [
      `ผู้ขอ: ${r.requested_by || '-'}`,
      `วันที่: ${formatPrDate(r.request_date)}`,
      `รายละเอียด: ${r.details || '-'}`,
      r.vendor ? `Vendor: ${r.vendor}` : null,
      r.notes ? `หมายเหตุ: ${r.notes}` : null,
      `สถานะ: ${PR_STATUS_LABEL[r.status] || r.status}`,
    ].filter(Boolean).join('\n');
    copyPlainText(summary, copyBtn);
  };

  $('prInfoModal').hidden = false;
  $('prInfoModal').classList.add('is-visible');
}

function closePrInfoCard() {
  $('prInfoModal').classList.remove('is-visible');
  $('prInfoModal').hidden = true;
}

function bindPrInfoCard() {
  $('closePrPixelCard').addEventListener('click', closePrInfoCard);
  $('prInfoModal').addEventListener('click', (e) => { if (e.target === $('prInfoModal')) closePrInfoCard(); });
  $('prPixelStatusBadge').addEventListener('click', (e) => openPrStatusMenu(e.currentTarget));
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && $('prInfoModal').classList.contains('is-visible')) closePrInfoCard();
  });
}
...
// เรียงลำดับรายการขอราคา (สำหรับตาราง)
function sortPriceRequests(list) {
  if (!prSortColumn) return list;
  const col = prSortColumn;
  const dir = prSortDirection === 'asc' ? 1 : -1;
...
function renderPriceRequests() {
  ...
  filtered = sortPriceRequests(filtered);
  ...
  renderPrTableRows(filtered, canDelete());
}

// ==================================================
// Dashboard สรุปข้อมูลงานขอราคา — อิงจากตัวกรองสถานะที่เลือกอยู่ (แต่ไม่อิงคำค้นหา เพื่อให้เห็นภาพรวมทั้งหมดเสมอ)
// ==================================================
function renderPrDashboard() {
  const all = allPriceRequests;
  const total = all.length;
  const pending = all.filter(r => r.status === 'pending').length;
  const inProgress = all.filter(r => r.status === 'in_progress').length;
  const done = all.filter(r => r.status === 'done').length;
  const overdue = getOverdueInProgressPr().length;
  const stale = getStalePriceRequests().length;
  const today = todayIso();
  const addedToday = all.filter(r => r.created_at && r.created_at.slice(0, 10) === today).length;
  const doneToday = all.filter(r => r.status === 'done' && r.status_changed_at && r.status_changed_at.slice(0, 10) === today).length;

  $('prDashboard').innerHTML = `
    <div class="insights-summary-card">
      <div class="insights-summary-value">${total}</div>
      <div class="insights-summary-label">งานขอราคาทั้งหมด</div>
    </div>
    <div class="insights-summary-card">
      <div class="insights-summary-value">${pending}</div>
      <div class="insights-summary-label">รอดำเนินการ</div>
    </div>
    <div class="insights-summary-card">
      <div class="insights-summary-value">${inProgress}</div>
      <div class="insights-summary-label">กำลังทำ</div>
    </div>
    <div class="insights-summary-card">
      <div class="insights-summary-value">${done}</div>
      <div class="insights-summary-label">เสร็จแล้ว</div>
    </div>
    <div class="insights-summary-card${overdue ? ' is-alert' : ''}">
      <div class="insights-summary-value">${overdue}</div>
      <div class="insights-summary-label">กำลังทำเกิน ${OVERDUE_IN_PROGRESS_HOURS} ชม.</div>
    </div>
    <div class="insights-summary-card${stale ? ' is-alert' : ''}">
      <div class="insights-summary-value">${stale}</div>
      <div class="insights-summary-label">ค้างเกิน ${STALE_PR_DAYS} วัน</div>
    </div>
    <div class="insights-summary-card">
      <div class="insights-summary-value">${addedToday}</div>
      <div class="insights-summary-label">เพิ่มวันนี้</div>
    </div>
    <div class="insights-summary-card">
      <div class="insights-summary-value">${doneToday}</div>
      <div class="insights-summary-label">เสร็จวันนี้</div>
    </div>
  `;
}
...
// ==================================================
// เรียงคอลัมน์ตารางขอราคา (เหมือนหน้าสินค้า) — ใช้ state/ฟังก์ชันแยกจากตาราง products
// เพื่อไม่ให้ชนกัน เพราะ #prTable ก็ใช้ class "product-table" ร่วมด้วยเพื่อความสวยงามที่ตรงกัน
// ==================================================
function bindPrSortEvents() {
  document.querySelectorAll('#prTable thead th.sortable').forEach(th => {
    th.addEventListener('click', () => {
      const col = th.dataset.sort;
      if (prSortColumn === col) {
...
function updatePrSortHeaderUI() {
  document.querySelectorAll('#prTable thead th.sortable').forEach(th => {
    th.classList.remove('is-sorted', 'is-sorted-asc', 'is-sorted-desc');
    if (th.dataset.sort === prSortColumn) {
      th.classList.add('is-sorted', prSortDirection === 'asc' ? 'is-sorted-asc' : 'is-sorted-desc');
    }
  });
}
...
function bindPrStatusMenu() {
  $('prStatusMenu').querySelectorAll('.pr-status-menu-item').forEach(item => {
    item.addEventListener('click', async () => {
      const targetId = prStatusMenuTargetId;
      closePrStatusMenu();
      if (!targetId) return;
      await setPrStatus(targetId, item.dataset.status);
      // ถ้า popup การ์ดของงานนี้เปิดค้างอยู่ รีเฟรชให้โชว์สถานะล่าสุดด้วย ไม่ให้ค้างข้อมูลเก่า
      if ($('prInfoModal').classList.contains('is-visible') && $('prPixelStatusBadge').dataset.id === targetId) {
        openPrInfoCard(targetId);
      }
    });
  });
...
// ==================================================
// CheckCost — ตรวจสอบราคาในตาราง (.xlsx / .xls / .csv)
// ทำงานในเบราว์เซอร์ล้วนๆ ผ่าน SheetJS (XLSX) ที่โหลดไว้อยู่แล้ว ไม่มีการเรียก API ภายนอกใดๆ
//
// กฎที่ตรวจสอบ (อ้างอิงจาก comment สีแดงในไฟล์ตัวอย่างที่ผู้ใช้แนบมา):
// 1) "2 Purchase Standard Price List" ต้องตรงกับ "Last Purchase Price" เสมอ —
//    และถ้า Last Purchase Price ไม่เท่ากับ 0/ว่างเปล่า คอลัมน์นี้ก็ห้ามว่างเปล่าเช่นกัน
//    (ครอบคลุมทั้งสองทิศทาง: ถ้า Last Purchase ว่าง/0 แต่คอลัมน์นี้ดันมีค่า ก็ถือว่าไม่ตรงกันเหมือนกัน)
// 2) "1 Sales Standard Price List" ห้ามว่างเปล่า และห้ามน้อยกว่า Last Purchase Price
//    หรือน้อยกว่า 2 Purchase Standard Price List
// ==================================================

const CHECKCOST_HEADER_PATTERNS = {
  itemNo: [/item\s*no/i, /รหัสสินค้า/i, /item\s*code/i],
  itemDesc: [/item\s*description/i, /รายละเอียดสินค้า/i, /^description$/i],
  lastPurchase: [/last\s*purchase\s*price/i],
  purchaseStd: [/purchase\s*standard\s*price/i], // ต้องเช็คหลัง lastPurchase เสมอ กัน "Last Purchase Price" ไปแมตช์คอลัมน์นี้ผิด
  salesStd: [/sales\s*standard\s*price/i],
};

// หาคอลัมน์ที่ตรงกับ pattern ที่กำหนด จากแถวหัวตาราง (rows[headerRowIndex])
// คืนค่า index ของคอลัมน์แรกที่แมตช์ หรือ -1 ถ้าไม่เจอ
function findCheckCostColumn(headerRow, patterns, excludeIndexes = []) {
...
// หาแถวที่น่าจะเป็น "แถวหัวตาราง" จริง — ไล่หาแถวแรกที่มีทั้งคอลัมน์ Last Purchase Price และ Sales Standard Price
// (กันกรณีมีแถวหัวเรื่อง/ว่างๆ อยู่ก่อนแถวหัวตารางจริง)
function findCheckCostHeaderRow(rows) {
  for (let r = 0; r < Math.min(rows.length, 15); r++) {
    const row = rows[r];
    if (!Array.isArray(row) || row.length === 0) continue;
    const hasLastPurchase = findCheckCostColumn(row, CHECKCOST_HEADER_PATTERNS.lastPurchase) !== -1;
    const hasSales = findCheckCostColumn(row, CHECKCOST_HEADER_PATTERNS.salesStd) !== -1;
    if (hasLastPurchase && hasSales) return r;
  }
  return -1;
}

function parseCheckCostPrice(raw) {
  if (raw === null || raw === undefined) return null;
  if (typeof raw === 'number') return Number.isFinite(raw) ? raw : null;
  const str = String(raw).trim();
  if (str === '' || str === '-') return null;
  const cleaned = str.replace(/THB/gi, '').replace(/฿/g, '').replace(/,/g, '').trim();
  if (cleaned === '') return null;
  const num = parseFloat(cleaned);
  return Number.isFinite(num) ? num : null;
}

const CHECKCOST_TOLERANCE = 0.01; // กันพลาดจากปัดเศษทศนิยม/floating point เล็กน้อย

// ตรวจสอบ 1 แถวตามกฎทั้งสองข้อ คืนค่าเป็น array ของข้อความ error (ว่าง = ไม่มีปัญหา)
function validateCheckCostRow({ lastPurchase, purchaseStd, salesStd }) {
  const errors = [];
  const lastPurchaseHasValue = lastPurchase !== null && Math.abs(lastPurchase) > CHECKCOST_TOLERANCE;

  // กฎที่ 1 — เช็คทิศทางเดียว: ตรวจเฉพาะตอน Last Purchase Price มีค่าจริง (ไม่ใช่ 0/ว่างเปล่า)
  // ถ้า Last Purchase Price ว่าง/เป็น 0 แต่ "2 Purchase Standard Price List" มีค่าอยู่ ถือว่าปกติ ไม่ต้องแจ้งเตือน
  // (เคสนี้เกิดได้ตามธรรมชาติ เช่น ตั้งราคาทุนไว้ล่วงหน้าแล้ว แต่ยังไม่เคยมีการซื้อจริงเกิดขึ้นเลย)
  if (lastPurchaseHasValue) {
    if (purchaseStd === null) {
      errors.push(`"2 Purchase Standard Price List" ว่างเปล่า ทั้งที่ Last Purchase Price = ${lastPurchase}`);
    } else if (Math.abs(purchaseStd - lastPurchase) > CHECKCOST_TOLERANCE) {
      errors.push(`"2 Purchase Standard Price List" (${purchaseStd}) ไม่ตรงกับ Last Purchase Price (${lastPurchase})`);
    }
  }

  // กฎที่ 2
  if (salesStd === null) {
    errors.push('"1 Sales Standard Price List" ว่างเปล่า');
  } else {
    if (lastPurchase !== null && salesStd < lastPurchase - CHECKCOST_TOLERANCE) {
      errors.push(`"1 Sales Standard Price List" (${salesStd}) น้อยกว่า Last Purchase Price (${lastPurchase})`);
    }
    if (purchaseStd !== null && salesStd < purchaseStd - CHECKCOST_TOLERANCE) {
      errors.push(`"1 Sales Standard Price List" (${salesStd}) น้อยกว่า 2 Purchase Standard Price List (${purchaseStd})`);
    }
  }
...
function bindCheckCostEvents() {
  const dropzone = $('checkCostDropzone');
  const fileInput = $('checkCostFileInput');
  const processBtn = $('checkCostProcessBtn');
  ...
  dropzone.addEventListener('drop', (e) => {
    e.preventDefault();
    dropzone.classList.remove('is-dragover');
    if (e.dataTransfer.files.length > 0) setCheckCostFile(e.dataTransfer.files[0]);
  });
  fileInput.addEventListener('change', () => {
    if (fileInput.files.length > 0) setCheckCostFile(fileInput.files[0]);
  });

  processBtn.addEventListener('click', processCheckCostFile);
  $('checkCostExportBtn').addEventListener('click', exportCheckCostErrors);
}

function setCheckCostFile(file) {
  checkCostFile = file;
  $('checkCostFileName').hidden = false;
  $('checkCostFileName').textContent = file.name;
  $('checkCostProcessBtn').disabled = false;
  setFieldError($('checkCostError'), '');
  $('checkCostResultWrap').hidden = true;
}

async function processCheckCostFile() {
  if (!checkCostFile) return;
  const btn = $('checkCostProcessBtn');
  const errorEl = $('checkCostError');
  setFieldError(errorEl, '');
  btn.disabled = true;
  btn.classList.add('is-loading');
  $('checkCostResultWrap').hidden = true;
  ...
  try {
    const fileName = checkCostFile.name.toLowerCase();
    const isCsv = fileName.endsWith('.csv');
    const arrayBuffer = await checkCostFile.arrayBuffer();
    let wb;
    if (isCsv) {
      // ไฟล์ CSV ต้อง decode เป็นข้อความแบบ UTF-8 ก่อนเสมอ ไม่งั้น SheetJS จะตีความ byte แบบ codepage เดิม (1252)
      // ทำให้ตัวอักษรไทย (multi-byte) กลายเป็นตัวอักษรมั่วๆ (เช่น "à¸«à¸™à¹‰à¸²à¸")
      const text = new TextDecoder('utf-8').decode(arrayBuffer);
      wb = XLSX.read(text, { type: 'string' });
    } else {
      // .xlsx/.xls เป็นไฟล์ binary ที่เก็บสตริงเป็น UTF-8 XML อยู่แล้วภายใน ไม่ต้อง decode เอง
      wb = XLSX.read(arrayBuffer, { type: 'array' });
    }
    const firstSheet = wb.Sheets[wb.SheetNames[0]];
    const rows = XLSX.utils.sheet_to_json(firstSheet, { header: 1, defval: '', raw: true });

    const headerRowIndex = findCheckCostHeaderRow(rows);
    if (headerRowIndex === -1) {
      throw new Error('หาแถวหัวตารางไม่เจอ — ไฟล์ต้องมีคอลัมน์ "Last Purchase Price" และ "...Sales Standard Price..." อยู่ในแถวเดียวกัน');
    }
    const headerRow = rows[headerRowIndex];

    const colItemNo = findCheckCostColumn(headerRow, CHECKCOST_HEADER_PATTERNS.itemNo);
    const colItemDesc = findCheckCostColumn(headerRow, CHECKCOST_HEADER_PATTERNS.itemDesc);
    const colLastPurchase = findCheckCostColumn(headerRow, CHECKCOST_HEADER_PATTERNS.lastPurchase);
    // "Purchase Standard Price" ต้องกันไม่ให้ไปแมตช์คอลัมน์เดียวกับ Last Purchase Price ซ้ำ
    const colPurchaseStd = findCheckCostColumn(headerRow, CHECKCOST_HEADER_PATTERNS.purchaseStd, [colLastPurchase]);
    const colSalesStd = findCheckCostColumn(headerRow, CHECKCOST_HEADER_PATTERNS.salesStd);

    const missing = [];
    if (colLastPurchase === -1) missing.push('Last Purchase Price');
    if (colPurchaseStd === -1) missing.push('2 Purchase Standard Price List');
    if (colSalesStd === -1) missing.push('1 Sales Standard Price List');
    if (missing.length > 0) {
      throw new Error(`หาคอลัมน์ไม่เจอ: ${missing.join(', ')} — กรุณาตรวจสอบชื่อหัวคอลัมน์ในไฟล์`);
    }
    ...
    dataRows.forEach((row, idx) => {
      ...
      const lastPurchase = parseCheckCostPrice(row[colLastPurchase]);
      const purchaseStd = parseCheckCostPrice(row[colPurchaseStd]);
      const salesStd = parseCheckCostPrice(row[colSalesStd]);
      checkedCount++;

      const errs = validateCheckCostRow({ lastPurchase, purchaseStd, salesStd });
      if (errs.length > 0) {
        errorRows.push({
          rowNumber: headerRowIndex + idx + 2, // เลขแถวจริงในไฟล์ต้นฉบับ (1-based, รวมแถวหัวตาราง) ไว้ให้ผู้ใช้ตามหาง่าย
          itemNo: colItemNo !== -1 ? String(row[colItemNo] ?? '').trim() : '',
          itemDesc: colItemDesc !== -1 ? String(row[colItemDesc] ?? '').trim() : '',
          lastPurchase, purchaseStd, salesStd,
          errors: errs,
        });
      }
    });

    checkCostErrorRows = errorRows;
    renderCheckCostResults(checkedCount, errorRows);
    showToast(`ตรวจสอบเสร็จแล้ว ${checkedCount} แถว พบข้อผิดพลาด ${errorRows.length} แถว`, errorRows.length ? 'default' : 'success');

    // ส่งออกเป็น Excel ให้อัตโนมัติทันทีที่ตรวจเสร็จ (ไม่ต้องกดปุ่มส่งออกซ้ำ) เพื่อให้เอาไปทำงานต่อ (กรอง/เรียง/ส่งต่อ) ได้เลย
    if (errorRows.length > 0) {
      exportCheckCostErrors();
    }
  } catch (err) {
    console.error('CheckCost error:', err);
    setFieldError(errorEl, err.message || 'ประมวลผลไฟล์ไม่สำเร็จ');
  } finally {
    btn.disabled = false;
    btn.classList.remove('is-loading');
  }
...
function renderCheckCostResults(checkedCount, errorRows) {
  const okCount = checkedCount - errorRows.length;
  const accuracy = checkedCount > 0 ? ((okCount / checkedCount) * 100).toFixed(1) : '0.0';

  $('checkCostSummary').innerHTML = `
    <div class="insights-summary-card">
      <div class="insights-summary-value">${checkedCount.toLocaleString('th-TH')}</div>
      <div class="insights-summary-label">แถวที่ตรวจสอบทั้งหมด</div>
    </div>
    <div class="insights-summary-card${errorRows.length ? ' is-alert' : ''}">
      <div class="insights-summary-value">${errorRows.length.toLocaleString('th-TH')}</div>
      <div class="insights-summary-label">แถวที่พบข้อผิดพลาด</div>
    </div>
    <div class="insights-summary-card">
      <div class="insights-summary-value">${okCount.toLocaleString('th-TH')}</div>
      <div class="insights-summary-label">แถวที่ถูกต้อง</div>
    </div>
    <div class="insights-summary-card">
      <div class="insights-summary-value">${accuracy}%</div>
      <div class="insights-summary-label">ความถูกต้อง</div>
    </div>
  `;

  const list = $('checkCostErrorList');
  const exportBtn = $('checkCostExportBtn');
  const allClear = $('checkCostAllClear');
  ...
  if (errorRows.length === 0) {
    ...
    list.innerHTML = errorRows.map(r => `
      <div class="checkcost-error-item">
        <div class="checkcost-error-item-head">
          <span class="checkcost-error-row">แถวที่ ${r.rowNumber}</span>
          ${r.itemNo ? `<span class="checkcost-error-itemno">${escapeHtml(r.itemNo)}</span>` : ''}
          ${r.itemDesc ? `<span class="checkcost-error-desc">${escapeHtml(r.itemDesc)}</span>` : ''}
        </div>
        <ul class="checkcost-error-reasons">
  ...
  $('checkCostResultWrap').hidden = false;
}

function exportCheckCostErrors() {
  if (checkCostErrorRows.length === 0) return;

  const rows = checkCostErrorRows.map(r => ({
    'แถวที่ (ในไฟล์ต้นฉบับ)': r.rowNumber,
    'Item No.': r.itemNo || '-',
    'Item Description': r.itemDesc || '-',
    'Last Purchase Price': r.lastPurchase ?? '-',
    '2 Purchase Standard Price List': r.purchaseStd ?? '-',
    '1 Sales Standard Price List': r.salesStd ?? '-',
    'ปัญหาที่พบ': r.errors.join(' | '),
  }));

  const sheet = XLSX.utils.json_to_sheet(rows);
  sheet['!cols'] = [
    { wch: 18 }, { wch: 16 }, { wch: 30 }, { wch: 16 }, { wch: 22 }, { wch: 20 }, { wch: 60 },
  ];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, sheet, 'CheckCost Errors');
  XLSX.writeFile(wb, `CheckCost_errors_${todayIso()}.xlsx`);
}
